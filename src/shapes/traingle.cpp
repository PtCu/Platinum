...
namespace platinum
{
    ...
    TriangleMesh::TriangleMesh(Transform *objectToWorld, const std::string &filename)
    {
    }

    //-------------------------------------------TriangleShape-------------------------------------

    PLT_REGISTER_CLASS(TriangleShape, "Triangle")

    TriangleShape::TriangleShape(const PropertyTreeNode &node)
    ...
    TriangleShape::TriangleShape(Transform *objectToWorld, Transform *worldToObject,
                                 std::array<int, 3> indices, TriangleMesh *mesh) : Shape(objectToWorld, worldToObject), m_indices(indices), m_mesh(mesh) {}
    ...
    Bounds3f TriangleShape::objectBound() const
    {
        ...
        return unionBounds(Bounds3f((*m_worldToObject)(p0, 1.0f), (*m_worldToObject)(p1, 1.0f)), (*m_worldToObject)(p2, 1.0f));
    }
    ...
    bool TriangleShape::hit(const ARay &ray, Float &tHit, SurfaceInteraction &isect) const
    {
        ...
        isect.n = Vector3f(normalize(cross(dp02, dp12)));
        tHit = t;

        if (m_mesh->hasNormal())
        {
            Vector3f ns;
            ns = b0 * m_mesh->getNormal(m_indices[0]) + b1 * m_mesh->getNormal(m_indices[1]) + b2 * m_mesh->getNormal(m_indices[2]);
    ...
    Float TriangleShape::solidAngle(const Vector3f &p, int nSamples) const
    {
        ...
        const auto &p1 = m_mesh->getPosition(m_indices[1]);
        const auto &p2 = m_mesh->getPosition(m_indices[2]);
        std::array<Vector3f, 3> pSphere = {normalize(p0 - p), normalize(p1 - p), normalize(p2 - p)};
