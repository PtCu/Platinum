...
namespace platinum
{
	...
	KdTree::KdTree(const std::vector<Hitable::ptr> &hitables, int isectCost /* = 80*/, int traversalCost /* = 1*/,
				   Float emptyBonus /* = 0.5*/, int maxHitables /* = 1*/, int maxDepth /* = -1*/) : m_isectCost(isectCost),
	...
	void KdTree::buildTree(int nodeIndex, const Bounds3f &nodeBounds,
