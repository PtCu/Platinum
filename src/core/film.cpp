...
namespace platinum
{
    ...
    PLT_REGISTER_CLASS(Film, "Film")

    Film::Film(const PropertyTreeNode &node)
    {
        const auto &props = node.getPropertyList();
        Vector2f _res = props.getVector2f("Resolution", Vector2f(800, 600));
        m_resolution = Vector2i(static_cast<int>(_res.x), static_cast<int>(_res.y));
        ...
        LOG(INFO) << "Created film with full resolution " << m_resolution << ". Crop window -> croppedPixelBounds " << m_croppedPixelBounds;
