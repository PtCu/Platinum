...
#define PCG32_MULT 0x5851f42d4c957f2dULL
    ...
    public:
        ...
        Random(uint64_t sequenceIndex) { setSequence(sequenceIndex); }
        ...
        {
            //[0, 1)
            return glm::min(OneMinusEpsilon, Float(uniformUInt32() * 2.3283064365386963e-10f));
        }
        ...
        int64_t operator-(const Random &other) const
        {
    ...
    inline Random::Random() : state(PCG32_DEFAULT_STATE), inc(PCG32_DEFAULT_STREAM) {}

    inline void Random::setSequence(uint64_t initseq)
    {
        state = 0u;
        inc = (initseq << 1u) | 1u;
        uniformUInt32();
    ...
    inline uint32_t Random::uniformUInt32()
    {
